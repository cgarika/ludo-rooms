...
(async () => {
  ...
  while (!done && Date.now() - t0 < 300000) {
    ...
    if (st && st.status === "playing" && st.mine) { console.log("turn hand=" + st.n + " phase=" + st.phase);
      if (st.n >= 12 && st.phase === "drawn") { global.k = (global.k || 0) + 1; const dbg = await p.evaluate(() => { const h = document.getElementById("hand"); const jd = h.querySelector(".hcard.justdrawn"); const r0 = { scrollLeft: h.scrollLeft, sw: h.scrollWidth, cw: h.clientWidth, jd: !!jd, jdLeft: jd ? jd.offsetLeft : null, jdIdx: jd ? [...h.children].indexOf(jd) : null, n: h.children.length, key: handScrollKey, drawnIdx: S.room.drawnIdx }; if (jd) { jd.scrollIntoView({ inline: "center", block: "nearest" }); r0.afterManual = h.scrollLeft; } return r0; }); console.log("drawn dbg", JSON.stringify(dbg)); await p.waitForTimeout(500); console.log("drawn dbg+500", await p.evaluate(() => document.getElementById("hand").scrollLeft)); if (global.k >= 3) { done = true; break; } }
      if (false) { await p.waitForTimeout(600); const info = await p.evaluate(() => ({ n: document.querySelectorAll("#hand .hcard").length, legal: document.querySelectorAll("#hand .hcard.legal").length, hint: document.getElementById("actionhint").textContent, scrollW: document.getElementById("hand").scrollWidth, clientW: document.getElementById("hand").clientWidth, pageW: document.documentElement.scrollWidth, viewW: innerWidth, gameW: document.getElementById("game").getBoundingClientRect().width })); console.log("big hand", JSON.stringify(info));
