...
function publicRoom(room) {
  return {
    ...
    players: room.players.map((p) => ({
      name: p.name, color: p.color, left: p.left, connected: p.connected, avatar: p.avatar,
    })),
...
io.on("connection", (socket) => {
  ...
  socket.on("create", ({ name, playerId, avatar } = {}) => {
    ...
    const room = {
      ...
      tokens: {}, winner: null, cfg: null, tokenChoice: 4,
      chat: [], lastMove: null,
  ...
  socket.on("settings", ({ tokens } = {}) => {
    const room = currentRoom();
    if (!room || room.status !== "lobby") return;
    if (room.host !== socket.data.playerId) return;
    if (![2, 3, 4].includes(tokens)) return;
